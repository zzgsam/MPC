...
if __name__ == '__main__':
    ...
    print(P_f)
    # Integrator
    inverval=T_var/N_pred
    for k in range(N_pred):
        st = X[:, k]
        con = u_pwm
        f_value = f(st, con)
       # print(f_value)
        st_next = st + (inverval* f_value)
        X[:, k + 1] = st_next
        cost_stage=((st - P[3:6]).T@Q@(st - P[3:6]) + (con-u_r).T@R@(con-u_r)+(st_next - P[3:6]).T@Q@(st_next - P[3:6]) + (con-u_r).T@R@(con-u_r))*inverval/2
        obj = obj + cost_stage #TODO:// Replace with another implementable Integrator
    print("obj",obj)
    st = X[:, N_pred]
    print(st)
    ...
    OPT_variables=vertcat(u_pwm,T_var)
    OPT_variables = reshape(OPT_variables,2 , 1)
    ...
    nl = {}
    nl['lbx'] = [0 for i in range(2)]
    nl['ubx'] = [0 for i in range(2)]
    nl['lbx'][0] = 0
    nl['ubx'][0] = 255
    nl['lbx'][1] = 0
    nl['ubx'][1] = inf
    ...
    while pred_counter <= 100:
        ...
        if (SX(state_0) - SX(state_r)).T@P_f@ (SX(state_0) - SX(state_r)) >= cons_sublevel:
            ...
            sol = solver(**nl)
            print(sol)
            x_temp_v = f(sol['g'][-7:-4], sol['x'][-2]) * sol['x'][-1] / N_pred + sol['g'][-7:-4]
            print(x_temp_v)
            print("norm",(x_temp_v-state_r).T@P_f@(x_temp_v-state_r))
